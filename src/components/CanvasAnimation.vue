<template>
    <canvas ref="canvas" width="400" height="400" style="border:1px solid #000;"></canvas>
  </template>
  ...
  /* ----- Функции для анимации (аналогичные вашему main.ts) ----- */
  ...
  function drawPolygonsTranslated(ctx, polygons, offsetFactor) {
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    polygons.forEach(poly => {
      const offsetX = poly.direction.x * offsetFactor;
      const offsetY = poly.direction.y * offsetFactor;
      ctx.beginPath();
      const first = poly.originalPoints[0];
      ctx.moveTo(first.x + offsetX, first.y + offsetY);
      for (let i = 1; i < poly.originalPoints.length; i++) {
        const pt = poly.originalPoints[i];
  ...
  function animateTranslation(ctx, polygons, maxOffset) {
    const period = 4000;
    ...
    function animate(currentTime) {
      ...
      drawPolygonsTranslated(ctx, polygons, offsetFactor);
      requestAnimationFrame(animate);
    }
    requestAnimationFrame(animate);
  }
  
  /* ----- Запуск анимации на канве ----- */
  onMounted(() => {
    ...
    const maxOffset = 15;
    animateTranslation(ctx, polygons, maxOffset);
  });
  </script>
